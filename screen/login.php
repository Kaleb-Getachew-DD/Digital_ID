...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Prepare and execute
    $stmt = $conn->prepare("SELECT password, position FROM user WHERE user_name = ?");
    $stmt->bind_param("s", $user_name);
    $stmt->execute();
    $stmt->store_result();

    if ($stmt->num_rows > 0) {
        $stmt->bind_result($db_password, $position);
        $stmt->fetch();
        ...
        if ($password === $db_password) {
            ...
            $stmt = $conn->prepare("UPDATE user SET status = '1' WHERE user_name = ?");
            $stmt->bind_param("s", $user_name);
            $stmt->execute();
          ...
          style="font-weight: 700;">Login Into Your Account</h5>
            ...
            <div class="col-mb-3">
                <label for="user_name"><i 
                  class="fa fa-envelope"></i> user name</label>
